...
public class QRReaderActivity extends AppCompatActivity implements ZXingScannerView.ResultHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(currentApiVersion >=  Build.VERSION_CODES.M)
        {
            if(checkPermission())
            {
                Toast.makeText(getApplicationContext(), "Scan Patient's QR Code", Toast.LENGTH_LONG).show();
            }
            else
            {
                requestPermission();
    ...
    public void handleResult(Result result) {
        final String patientInfo = result.getText();
        //TODO: send notification to others here
        ...
        builder.setNeutralButton("Visit", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(patientInfo));
